/*****************************************************************************************
 * Source File: ApplyRefresh.java
 ****************************************************************************************/
package net.ruready.business.common.tree.entity;

import net.ruready.common.misc.Auxiliary;
import net.ruready.eis.common.tree.LimitedDepthTreeVisitor;

/**
 * Apply a refresh operation to an entire tree. This re-sorts children and synchronizes
 * their depth fields with the root node.
 * <p>
 * -------------------------------------------------------------------------<br>
 * (c) 2006-2007 Continuing Education, University of Utah<br>
 * All copyrights reserved. U.S. Patent Pending DOCKET NO. 00846 25702.PROV
 * <p>
 * This file is part of the RUReady Program software.<br>
 * Contact: Nava L. Livne <code>&lt;nlivne@aoce.utah.edu&gt;</code><br>
 * Academic Outreach and Continuing Education (AOCE)<br>
 * 1901 East South Campus Dr., Room 2197-E<br>
 * University of Utah, Salt Lake City, UT 84112-9359<br>
 * U.S.A.<br>
 * Day Phone: 1-801-587-5835, Fax: 1-801-585-5414<br>
 * <br>
 * Please contact these numbers immediately if you receive this file without permission
 * from the authors. Thank you.<br>
 * -------------------------------------------------------------------------
 * 
 * @author Oren E. Livne <code>&lt;olivne@aoce.utah.edu&gt;</code>
 * @version Aug 11, 2007
 */
@Deprecated
class ApplyRefresh extends LimitedDepthTreeVisitor implements Auxiliary
{
	// ========================= FIELDS =====================================

	// ========================= CONSTRUCTORS ==============================

	/**
	 * Create an application object.
	 */
	private ApplyRefresh(Node thisNode)
	{
		super();
		// Initialize depth to the tree node depth
		setDepth(thisNode.getDepth());
	}

	/**
	 * A facade to be called instead of constructing this object. Refreshes a tree.
	 * 
	 * @param thisNode
	 *            tree root node
	 */
	public static void refresh(Node thisNode)
	{
		new ApplyRefresh(thisNode).executeOnTree(thisNode);
	}

	// ========================= IMPLEMENTATION: LimitedDepthTreeVisitor =============

	/**
	 * @param thisNode
	 * @return
	 * @see net.ruready.eis.common.tree.LimitedDepthTreeVisitor#executePre(net.ruready.business.common.tree.entity.Node)
	 */
	@Override
	protected Object executePre(Node thisNode)
	{
		return null;
	}

	/**
	 * @see net.ruready.eis.common.tree.LimitedDepthTreeVisitor#executePost(net.ruready.business.common.tree.entity.Node)
	 */
	@Override
	protected Object executePost(Node thisNode)
	{
		// Use post-traversal ordering: refresh children before
		// refreshing a node
		// thisNode.refresh();
		thisNode.setDepth(getDepth());
		return null;
	}
}
