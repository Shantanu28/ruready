...
public class StrutsUtil implements Utility {
     ...
     * @see
     * http://www.roguewave.com/support/docs/hydraexpress/3.5.0/html/rwsfservletug/4-3.html
     * @param urlToMatch uri to match
     ...
     *
     * @param forward action forward object (will be cloned upon returning from
     * this method)
     ...
     * <code>false</code>, provided that the form bean's <code>validate()</code>
     * method calls this method (<code>isMethodValidated</code>) at its
     * beginning. This method returns <code>false</code> if and only if the
     * request contains a <code>WebAppNames.ACTION.VALIDATE_TOKEN</code>
     ...
     * the value <code>WebAppNames.ACTION.VALUE_FALSE</code>, the method returns
     * <code>false</code>. If its value is
     * <code>WebAppNames.ACTION.VALUE_TRUE</code>, the method return
     * <code>true</code>. In all other cases, <code>true</code> is returned.
     *
     * @param request
     ...
     * <code>WebAppNames.ACTION.DISPATCH_PARAMETER_PREFIX</code> prefix from the
     * request parameter's name.
     *
     * @param request client's request
     * @return method name to be dispatched to or <code>null</code> if no
     * parameter matches the
     * <code>WebAppNames.ACTION.DISPATCH_PARAMETER_PREFIX</code> prefix.
     */
    @Deprecated
     ...
     *
     * @param request client's request
     * @return <code>true</code> iff the method name to be dispatched to is not
     * to be validated, i.e. starts with
     ...
     * @param actionName name of action method; does not include the common
     * action name prefix. For instance, if the action parameter value is
     * <code>&quot;action_setup_method&quot;</code>,
     * <code>actionName = &quot;method&quot;</code>; or if the action parameter
     * value is <code>&quot;action_method&quot;</code>,
     ...
     * @param required a boolean indicating whether or not input is required for
     * this field. Setting this indicator to false will cause the generation of
     * an additional option for no selection.
     * @param selectedId selected value's id
     ...
     * this field. Setting this indicator to false will cause the generation of
     * an additional option for no selection.
     * @param emptySelectionValue value of empty selection to use if required =
     * <code>false</code>
     * @param selectedId selected value's id
     * @return selection list
     */
     ...
     *
     * @param options list of options (generated by the OLS framework)
     * @param messageResources resource bundle
     * @param noSelectionKey key to use for i18n message of the no-selection
     * options. The labels of all such options will be overridden.
     * @return modified option list
     */
    public static OptionList i18NEmptySelectionOptions(final OptionList options,
            final MessageSource messageResources, final String noSelectionKey) {
        // Modify empty selection label for internationalization
        for (Option vl : options) {
            if (vl.isEmpty()) {
                // Found empty selection key, override label
                vl.setLabel(messageResources.getMessage(noSelectionKey, null, "", null));
            }
        }
        return options;
    }
     ...
     *
     * @param options existing options to i18nalize whose values are enumerated
     * type class names
     * @param clazz enumerated class type
     * @param required if false, will add an empty selection to the option list
     * @param prefix
     * @param messageResources resource bundle
     */
...
//                        + separator + vl.getValue() + separator + suffix));
                vl.setLabel(messageResources.getMessage(clazz.getCanonicalName()
                        + separator + vl.getValue() + separator + suffix, null, "", null));                
            }
        }
    }
    ...
    public static OptionList i18NSelection(Class<?> clazz, boolean required,
            String suffix, String separator, MessageSource messageResources) {
    ...
    public static <E extends Enum<E>> Map<E, String> i18NMap(final Class<E> clazz,
        ...
        for (E item : EnumSet.allOf(clazz)) {
            ...
            map.put(item, messageResources.getMessage(clazz.getCanonicalName()
                    + separator + item.toString() + separator + suffix, null, "", null));
//            map.put(item, messageResources.getMessage(clazz.getCanonicalName()
//                    + separator + item.toString() + separator + suffix));            
     ...
     *
     * @param groupId id of the select group
     * @param options list drop-down menu options
