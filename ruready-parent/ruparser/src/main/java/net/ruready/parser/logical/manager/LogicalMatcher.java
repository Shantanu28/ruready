...
/**
 * This class provides a parser that parsers logical expressions according to
 ...
 * @version Aug 16, 2007
 */
public class LogicalMatcher implements AbstractMatcher
{
	// ========================= CONSTANTS =================================
	...
	private static final Log logger = LogFactory.getLog(LogicalMatcher.class);
	...
	// the parser generated by the logical compiler
	...
	// -------------------------------------------------
	// target object
	private MathTarget logicalTarget;
	...
	/**
	 * Initialize an logical parser from options.
	 * 
	 * @param options
	 ...
	 *            the parser generated by the logical compiler
	 */
	public LogicalMatcher(ParserOptions options, Parser parser)
	{
		super();
	...
	public void match(String s)
	{
		...
		// an logical target that has a syntax tree and some other fields.
		// ------------------------------------------------------------------
		...
		if (out != null)
		{
			...
			logicalTarget = (MathTarget) out.getTarget();
			if (logicalTarget.hasErrors())
			{
				completeMatch = false;
				// Exception during assembly, throw an exception describing
				// the cause of failure
				InternationalizableErrorMessage error = logicalTarget
						.getFirstSyntaxErrorMessage();
				logger.debug("Found syntax errors: " + error.getMessage());
				throw new MathParserException(s, error);
			}
		...
		logger.debug(CommonNames.MISC.NEW_LINE_CHAR
				+ logicalTarget.toStringDetailed());

		// new
		// TreeReferenceInspector().executeOnTree(logicalTarget.getSyntax());
