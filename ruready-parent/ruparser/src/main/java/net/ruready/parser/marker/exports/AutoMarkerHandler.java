/*******************************************************
 * Source File: AutoMarkerHandler.java
 *******************************************************/
...
/**
 * A prepared parser processing chain that decides which marker to run based on
 * the parser options at run time, and then runs the chosen marker on a pair of
 ...
 * @version Apr 17, 2007
 */
public class AutoMarkerHandler extends RequestHandler
{
	// ========================= CONSTANTS =================================
	...
	private static final Log logger = LogFactory.getLog(AutoMarkerHandler.class);
	 ...
	 */
	public AutoMarkerHandler(RequestHandler nextNode)
	{
		super(nextNode);
	}
	...
	final protected boolean handle(ChainRequest request)
	{
		...
		switch (options.getAnalysisID())
		{
			...
			{
				// Decide where to forward the request next based on the options
				dynamicNextNode = new MarkerHandler(
						ParserNames.REQUEST.ATTRIBUTE.MARKER.ATPM_RESULT, this
								.getNextNode());
				break;
			}
		}
