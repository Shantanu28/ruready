/*****************************************************************************************
 * Source File: SingleLogicalEvaluationHandler.java
 ****************************************************************************************/
...
import net.ruready.parser.evaluator.manager.LogicalRounder;
import net.ruready.parser.evaluator.manager.Rounder;
 ...
 * @version Apr 16, 2007
 */
public class SingleLogicalEvaluationHandler extends ParserHandler
{
	// ========================= CONSTANTS =================================
	...
	private static final Log logger = LogFactory
			.getLog(SingleLogicalEvaluationHandler.class);
	 ...
	 */
	public SingleLogicalEvaluationHandler()
	{
		super();
	}
	 ...
	 */
	public SingleLogicalEvaluationHandler(RequestHandler nextNode)
	{
		super(nextNode);
	}
	...
	protected boolean handle(ChainRequest request)
	{
		...
		Evaluator<SyntaxTreeNode> evaluator = new LogicalEvaluator(
				options.getVariables(), true);
		// logger.debug("tree " + tree);
		SyntaxTreeNode result = evaluator.evaluate(tree);
		// Round evaluation result to tolerance specified by options
		Rounder<SyntaxTreeNode> rounder = new LogicalRounder(options);
		rounder.round(result);
