/*****************************************************************************************
 * Source File: SpecialFunctions.java
 ****************************************************************************************/
package net.ruready.common.math.real;

/**
 * NOTE: Taken from JSci package which has a LGPL license. As we only use very little of
 * this class and we've had our own version, get rid of most of it later on. The special
 * function math library. This class cannot be subclassed or instantiated because all
 * methods are static. (c) 2006-07 Continuing Education , University of Utah . All
...
public final class SpecialFunctions
{
	...
	/**
	 * Airy modulus and phase. Based on the NETLIB Fortran subroutine r9aimp written by W.
	 * Fullerton.
	 * 
	...
	/**
	 * Exponential scaled Airy function. Based on the NETLIB Fortran function aie written
	 * by W. Fullerton.
	 */
	...
	/**
	 * Bessel function of first kind, order zero. Based on the NETLIB Fortran function
	 * besj0 written by W. Fullerton.
	 */
	...
	/**
	 * Modified Bessel function of first kind, order zero. Based on the NETLIB Fortran
	 * function besi0 written by W. Fullerton.
	 */
	...
	/**
	 * Exponential scaled modified Bessel function of first kind, order zero. Based on the
	 * NETLIB Fortran function besi0e written by W. Fullerton.
	 */
	...
	/**
	 * Bessel function of first kind, order one. Based on the NETLIB Fortran function
	 * besj1 written by W. Fullerton.
	 */
	...
	/**
	 * Modified Bessel function of first kind, order one. Based on the NETLIB Fortran
	 * function besi1 written by W. Fullerton.
	 */
	...
	/**
	 * Exponential scaled modified Bessel function of first kind, order one. Based on the
	 * NETLIB Fortran function besi1e written by W. Fullerton.
	 */
	private static double expModBesselFirstOne(double x)
	{
		...
		if (y > 3.0)
		{
			if (y > 8.0)
				...
				return x / y * (0.375 + chebyshev((48.0 / y - 11.0) / 5.0, ai1cs))
						/ Math.sqrt(y);
		}
		else if (y == 0.0)
			return 0.0;
	...
	/**
	 * Bessel function of second kind, order zero. Based on the NETLIB Fortran function
	 * besy0 written by W. Fullerton.
	 */
	...
	/**
	 * Bessel function of second kind, order one. Based on the NETLIB Fortran function
	 * besy1 written by W. Fullerton.
	 */
	...
	private final static double LOGSQRT2PI = Math.log(RealConstants.SQRT2PI);

	// Gamma function related constants
	private final static double g_p[] =
	{
	...
	private final static double g_q[] =
	{
	...
	private final static double g_c[] =
	{
	...
	/**
	 * The largest argument for which <code>gamma(x)</code> is representable in the
	 * machine.
	 */
	public final static double GAMMA_X_MAX_VALUE = 171.624;

	/**
	 * Gamma function. Based on public domain NETLIB (Fortran) code by W. J. Cody and L.
	 * Stoltz<br>
	 ...
	 * References:
	 * <ol>
	 * <li>"An Overview of Software Development for Special Functions", W. J. Cody,
	 * Lecture Notes in Mathematics, 506, Numerical Analysis Dundee, 1975, G. A. Watson
	 ...
	 * </p>
	 * <p>
	 * This routine calculates the GAMMA function for a real argument X. Computation is
	 * based on an algorithm outlined in reference 1. The program uses rational functions
	 * that approximate the GAMMA function to at least 20 significant decimal digits.
	 * Coefficients for the approximation over the interval (1,2) are unpublished. Those
	 * for the approximation for X .GE. 12 are from reference 2. The accuracy achieved
	 * depends on the arithmetic system, the compiler, the intrinsic functions, and proper
	...
	/**
	 * The largest argument for which <code>logGamma(x)</code> is representable in the
	 * machine.
	 */
	...
	private final static double lg_p1[] =
	{
	...
	private final static double lg_q1[] =
	{
	...
	private final static double lg_p2[] =
	{
	...
	private final static double lg_q2[] =
	{
	...
	private final static double lg_p4[] =
	{
	...
	private final static double lg_q4[] =
	{
	...
	private final static double lg_c[] =
	{
			-0.001910444077728, 8.4171387781295e-4, -5.952379913043012e-4,
			7.93650793500350248e-4, -0.002777777777777681622553,
			0.08333333333333333331554247, 0.0057083835261
	...
	/**
	 * The natural logarithm of the gamma function. Based on public domain NETLIB
	 * (Fortran) code by W. J. Cody and L. Stoltz<br>
	 * Applied Mathematics Division<br>
	 * Argonne National Laboratory<br>
	 * Argonne, IL 60439<br>
	 * <p>
	 ...
	 * Computation is based on an algorithm outlined in references 1 and 2. The program
	 * uses rational functions that theoretically approximate LOG(GAMMA) to at least 18
	 * significant decimal digits. The approximation for X > 12 is from reference 3, while
	 * approximations for X < 12.0 are similar to those in reference 1, but are
	 * unpublished. The accuracy achieved depends on the arithmetic system, the compiler,
	 * the intrinsic functions, and proper selection of the machine-dependent constants.
	 * </p>
	 * <p>
	 * Error returns:<br>
	 * The program returns the value XINF for X .LE. 0.0 or when overflow would occur. The
	...
	/**
	 * Incomplete gamma function. The computation is based on approximations presented in
	...
	/**
	 * Incomplete beta function. The computation is based on formulas from Numerical
	 * Recipes, Chapter 6.4 (W.H. Press et al, 1992).
	 * 
	 * @param x
	 *            require 0<=x<=1
	 * @param p
	 ...
	 * @param q
	 *            require q>0
	 * @return 0 if x<0, p<=0, q<=0 or p+q>2.55E305 and 1 if x>1 to avoid errors and
	 *         over/underflow
	 * @author Jaco van Kooten
	 */
	public static double incompleteBeta(double x, double p, double q)
	{
		...
		{
			final double beta_gam = Math.exp(-logBeta(p, q) + p * Math.log(x) + q
					* Math.log(1.0 - x));
			if (x < (p + 1.0) / (p + q + 2.0))
	...
	/**
	 * Evaluates of continued fraction part of incomplete beta function. Based on an idea
	 * from Numerical Recipes (W.H. Press et al, 1992).
	 * 
	 * @author Jaco van Kooten
	 */
	...
	public static double error(double x)
	{
		...
		{ // 0.84375 < |x| < 1.25
			...
			P = ePa[0]
					...
					* (ePa[1] + s
							* (ePa[2] + s
									* (ePa[3] + s * (ePa[4] + s * (ePa[5] + s * ePa[6])))));
			Q = 1.0
					+ s
					* (eQa[0] + s
							* (eQa[1] + s
									* (eQa[2] + s * (eQa[3] + s * (eQa[4] + s * eQa[5])))));
			retval = e_erx + P / Q;
		}
		else if (abs_x >= 6.0)
			retval = 1.0;
	...
	/**
	 * Complementary error function. Based on C-code for the error function developed at
	 * Sun Microsystems.
	 * 
	 * @author Jaco van Kooten
	 */
	...
	{
		return Math.exp(logFact(n) - logFact(r));
	}

	/**
	 * Returns the base-<code>b</code> logarithm of a double value. Special cases:<br>
	 * If the argument is NaN or less than zero, then the result is NaN. If the argument
	 * is positive infinity, then the result is positive infinity. If the argument is
	 * positive zero or negative zero, then the result is negative infinity. If the base
	 * is negative, returns NaN. A result must be within 1 ulp of the correctly rounded
	 ...
	 * @param x
	 *            a number greater than 0.0
	 * @return the value ln<b>x, the base-<code>b</code> logarithm of x
	 */
	public static final double log(double b, double x)
	{
		return Math.log(x) / Math.log(b);
	}

	/**
	 * Returns the <code>b</code><i>th</i> root of a double value.
	 * 
	 * @param r
	 *            root order
	 * @param x
	 ...
	 */
	public static final double root(double r, double x)
	{
		return Math.pow(x, 1.0d / r);
	}

	/**
	 * Returns the co-secant of a double value (1/sin).
	 * 
	 * @param angleInRadians
	 *            an angle in radians
	 * @return the co-secant of <code>angleInRadians</code>.
	 */
	...
	/**
	 * Returns the co-secant of a double value (1/cos).
	 * 
	 * @param angleInRadians
	 *            an angle in radians
	 * @return the secant of <code>angleInRadians</code>.
	 */
	...
	/**
	 * Returns the co-tangent of a double value (1/tan).
	 * 
	 * @param angleInRadians
	 *            an angle in radians
	 * @return the co-tangent of <code>angleInRadians</code>.
	 */
