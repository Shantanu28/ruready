/*****************************************************************************************
 * Source File: HungarianAlgorithmEdu.java
 ****************************************************************************************/
...
/**
 * Started on Apr 25, 2005 Munkres-Kuhn (Hungarian) Algorithm Edu Version: 0.11
 ...
 * http://www.spatial.maine.edu/~kostas This Java class implements the Hungarian
 * algorithm [a.k.a Munkres' algorithm, a.k.a. Kuhn algorithm, a.k.a. Assignment
 * problem, a.k.a. Marriage problem, a.k.a. Maximum Weighted Maximum Cardinality
 * Bipartite Matching]. This is an educational version of the algorithm,
 * intended to help those who would like to understand how the algorithm
 * internally works. [I do NOT explain WHY the algorithm does the things it
 * does, just WHAT it does.] If you want to get rid of the comments and tutorial
 * character of this program, get the clean Hungarian algorithm from the same
 * website you got this one
 * (http://www.spatial.maine.edu/~kostas/dev/soft/munkres.htm). The clean
 * version can be used as a method call from within any main or other function.
 ...
 * this algorithm (http://216.249.163.93/bob.pilgrim/445/munkres.html) Feel free
 * to redistribute this source code, as long as this header--with the exception
 * of sections in brackets--remains as part of the file. Requirements: JDK
 * 1.5.0_01 or better. [Created in Eclipse 3.1M6 (www.eclipse.org).]
 * <p>
 * -------------------------------------------------------------------------<br>
...
public class HungarianAlgorithmEdu
{
	...
	private static final Log logger = LogFactory.getLog(HungarianAlgorithmEdu.class);
	...
	public static void insertLines(int blancLines) // Inserts <blancLine>
	// lines.
	{
	...
	// headers.
	{
		String stars = CommonNames.MISC.EMPTY_STRING;
	...
	public static double getTime(double start, double end) // Calculates
	// elapsed time in
	// seconds.
	{
		double total = (end - start) / 1000000000.0; // Convert time in
		// seconds
		return total;
	}
	...
	{
		String timeElapsed = CommonNames.MISC.EMPTY_STRING;
		...
		for (int i = 0; i < array.length; i++)
		{
			for (int j = 0; j < array[i].length; j++)
			{
				...
				if (randomMethod.equals("gaussian"))
				{
					...
					array[i][j] = array[i][j] + 0.5; // make elements
					// positive.
				}
			}
		}
	...
	public static double findLargest // Finds the largest element in a
	// positive array.
	(double[][] array)
	...
	public static int[][] hgAlgorithm(double[][] array, String sumType)
	{
		...
		{
			logger.debug("Since you want to find the assignment\n"
		...
		logger.debug("Algorithm out of main loop.\nProcess exited with code " + step
				+ ".\n");
	 ...
	 * @return
	 */
	public static int hg_step1(final int previousStep, double[][] cost)
	{
		...
		logger.debug("Step 1 finds the smallest value in each\n"
		...
		step = 2;
		logger.debug("Step 1 completed. Going to step " + step + ".\n\n");

		return step;
	}

	public static int hg_step2(final int previousStep, double[][] cost, int[][] mask,
			int[] rowCover, int[] colCover)
	{
		...
		logger.debug("Step 2 examines the cost matrix and sets\n"
		...
		for (int i = 0; i < cost.length; i++)
		{
			for (int j = 0; j < cost[i].length; j++)
			{
				...
				{
					logger.debug("The element cost(" + (i + 1) + "," + (j + 1)
							+ ") is 0...also\n" + "row " + (i + 1)
							+ " is uncovered and so is col " + (j + 1) + ".");
					mask[i][j] = 1;
					logger.debug("So we set mask(" + (i + 1) + "," + (j + 1)
							+ ") to 1 (starred).");
					colCover[j] = 1;
					rowCover[i] = 1;
		...
		step = 3;
		logger.debug("Step 2 completed. Going to step " + step + ".\n\n");
	 ...
	 * @return
	 */
	public static int hg_step3(final int previousStep, int[][] mask, int[] colCover)
	{
		int step = previousStep;
		// What STEP 3 does:
		...
		logger.debug("Step 3 covers columns of starred zeros.\n" + "If " + mask.length
				+ " columns are covered we are done.\n" + "(" + mask.length
				+ " because rows of cost matrix are " + mask.length + ".)\n"
		...
		{
			step = 7;
			logger.debug("There are " + count + " covered columns,\n"
		...
		else
		{
			...
			if (count == 1)
			{
				logger.debug("There is " + count + " covered column,\n"
			...
			logger.debug("Step 3 completed. Going to step " + step + ".\n\n");
		}
	 ...
	 * @return
	 */
	public static int hg_step4(final int previousStep, double[][] cost, int[][] mask,
			int[] rowCover, int[] colCover, int[] zero_RC)
	{
		...
		logger.debug("Step 4 finds uncovered zeros in the cost\n"
		...
		while (done == false)
		{
			...
			if (row_col[0] == -1)
			{
				logger.debug("All zeros are covered. Going to step 6.\n");
				done = true;
				step = 6;
			}
			else
			{
				logger.debug("There is an uncovered zero at: cost(" + (row_col[0] + 1)
						+ "," + (row_col[1] + 1) + ").");

				mask[row_col[0]][row_col[1]] = 2; // Prime the found uncovered
				// zero.

				logger.debug("Therefore, element cost(" + (row_col[0] + 1) + ","
						+ (row_col[1] + 1) + ") is primed.");
				...
				if (starInRow == true)
				{
					...
					logger.debug("It is element cost(" + (row_col[0] + 1) + ","
							+ (row_col[1] + 1) + ")\n"
					...
					logger.debug("Now we search again for uncovered zeros.");
				}
				else
				{
					logger.debug("There is no starred zero in the cost matrix\n"
							+ "in the same row with the zero we just primed.");
					done = true;
					step = 5;
					logger.debug("Therefore, we need to go to step " + step + ".\n"
					...
					logger.debug("The Zero_RC matrix now is:" + " [" + (zero_RC[0] + 1)
							+ "][" + (zero_RC[1] + 1) + "]\n");
				}
			}
		}

		logger.debug("Step 4 completed. Going to step " + step + ".\n\n");
		return step;
	}
	...
	{
		logger.debug("Finding an uncovered zero in cost matrix...");
	 ...
	 * @return
	 */
	public static int hg_step5(int step, int[][] mask, int[] rowCover, int[] colCover,
			int[] zero_RC)
	{
		// What STEP 5 does:
		...
		logger.debug("A summary of what this step does,\n"
		...
		logger.debug("We create the path matrix, and set its\n"
		...
		while (done == false)
		{
			...
			{
				logger.debug("No starred zero exists in column " + (path[count][1] + 1)
		...
		logger.debug("The Cover arrays are being cleared...");
		clearCovers(rowCover, colCover);
		logger.debug("Did it.");
		...
		int nextStep = 3;
		logger.debug("Step 5 completed. Going to step " + nextStep + ".\n\n");
	...
	(int[][] mask, int col)
	{
		logger.debug("Now we try to find whether a starred zero\n"
		...
		if (r != -1)
		{
			logger.debug("A starred zero does exist in column " + (col + 1) + ".\n"
					+ "It is in row " + (r + 1) + ".");
		}
	...
	(int[][] mask, int row)
	{
		logger.debug("Now we try to find whether a primed zero\n"
		...
		logger.debug("A primed zero does exist in row " + (row + 1) + ".\n"
				+ "It is in column " + (c + 1) + ".");
		return c;
	}
	...
	{
		logger.debug("convertPath() method now starts...\n"
	...
	(int[][] mask)
	{
		logger.debug("erasePrimes() method erases any other\n"
		...
		if (tempCount == 0)
		{
			logger.debug("No other primes were found.");
		}
		else
		{
			logger.debug(tempCount + " other primes were found and erased.\n");
		}
	}
			...
			double maxCost)
	{
		...
		logger.debug("Step 6 finds the smallest uncovered value\n"
				+ "in the cost matrix.\n"
		...
		int nextStep = 4;
		logger.debug("\nStep 6 completed. Going to step " + nextStep + ".\n\n");
		return nextStep;
	}
	...
	public static void main(String[] args)
	{
		...
		/*
		 * double[][] array = { {23, 27, 42, 55, 97}, {14, 94, 5, 77, 22}, {53,
		 * 3, 88, 89, 7}, {99, 52, 14, 1, 63}, {50, 100, 34, 41, 92} };
		 */

		/*
		 * //<UNCOMMENT> BELOW AND COMMENT BLOCK ABOVE TO USE A RANDOMLY
		 * GENERATED MATRIX //Do NOT enter any dimension larger than 5 or
		 * so...it will take long to finish //plus...you won't understand much
		 * because of the crowded output. int numOfRows = readInput("How many
		 * rows for the matrix? "); int numOfCols = readInput("How many columns
		 * for the matrix? "); double[][] array = new
		 * double[numOfRows][numOfCols]; generateRandomArray(array, "random");
		 * //All elements within [0,1].
		 */// </UNCOMMENT>
		logger.debug("INITIAL DATA MATRICES");
		System.out
				.println("(In all calcs to follow, printing\nonly 1 decimal digit for doubles)\n");
		if (array.length > array[0].length)
		{
			// Cols must be >= Rows.
			logger.debug("You entered a rectangular array where\n"
		...
		{
			arrayType = CommonNames.MISC.EMPTY_STRING;
		}

		print2DArray(array, "%.1f\t", ("The initial " + arrayType + " array was:"), 1);
		logger.debug("The winning assignment (" + sumType + " sum) is:\n");
		double sum = 0;
		for (int i = 0; i < assignment.length; i++)
		{
			...
			System.out.printf("array(%d,%d) = %.1f\n", (assignment[i][0] + 1),
					(assignment[i][1] + 1), array[assignment[i][0]][assignment[i][1]]);
		...
		logger.debug("Total time required: ");
		printTime(totalTime);
		insertLines(1);
