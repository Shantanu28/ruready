/*******************************************************
 * Source File: StatFunctions.java
 *******************************************************/
 ...
 *         University of Utah Protected by U.S. Provisional Patent U-4003,
 *         February 2006 Protected by U.S. Provisional Patent U-4003, February
 *         2006
 * @version Feb 23, 2006 Statistical functions. Source: Sundar Dorai-Raj <i><sdoraira@vt.edu><i>
...
public final class StatFunctions
{
	...
	private static final Log logger = LogFactory.getLog(StatFunctions.class);
	...
	{
		/*
		 * Reference: J. D. Beasley and S. G. Springer Algorithm AS 111: "The
	 ...
	 * @return
	 */
	public static double pnorm(final double z0, final boolean upper0)
	{
		double z = z0;
		boolean upper = upper0;
		/*
	...
	public static double qt(double p, double ndf, boolean lower_tail)
	{
		...
		{ /*-- usual case;  including, e.g.,  df = 1.1 */
			...
			if (y > 0.05 + a)
			{
				...
				y = a * y * y;
				if (y > 0.002) /*
								 * Note: This cutoff is machine-precision
								 * dependent.
								 */
					y = Math.exp(y) - 1;
				else
	...
	public double pchisq(double q, double df)
	{
		...
		if (q <= 0 || df <= 0)
			throw new IllegalArgumentException("Illegal argument " + q + " or " + df
					+ " for qnorm(p).");
		if (q < df)
		{
			...
			prob = Math.exp(df2 * Math.log(q2) - q2 - SpecialFunctions.logGamma(df2 + 1)
					- Math.log(CFL));
		}
		else
		{
			...
			prob = 1 - Math.exp((df2 - 1) * Math.log(q2) - q2
					- SpecialFunctions.logGamma(df2) - Math.log(CFU));
		}
		return prob;
	}
	...
	public static double betainv(double x, double p, double q)
	{
		...
		// Computes P(Beta>x)
		double beta = SpecialFunctions.logBeta(p, q), acu = 1E-14;
