...
/**
 * A TokenString is like a String, but it is a series of Tokens rather than a
 * series of chars. Once a TokenString is created, it is "immutable", meaning it
 * cannot change. This lets you freely copy TokenStrings without worrying about
 * their state.
 * 
...
public class TokenString
{
	 ...
	 * @param tokens
	 *            the tokens to use
	 * @return a tokenString constructed from the supplied tokens
	 */
	 ...
	 * @return a tokenString constructed from tokens read from the supplied
	 *         string
	 */
	 ...
	 * 
	 * @param Tokenizer
	 *            the tokenizer that will produces the tokens
	 * @return a tokenString constructed from the tokenizer's tokens
	 */
	public TokenString(Tokenizer t)
	{
		...
		tokens = v.toArray(new Token[v.size()]);
	}
	 ...
	 * 
	 * @param index
	 *            the index of the desired token
