...
Document-type Item custom editing form. This is not a full HTML page and included in
other edit pages inside an <html:form> tag.
###################################################################################
--%>
...
<tbody>
	...
	<%-- Display a text area for the document's content --%>
	<tr valign="top">
		<td>
			<bean:message key="content.DOCUMENT.content.label" />
		</td>
		<td>
			<nested:textarea property="content" rows="30" styleClass="documentcontent" />
			<content:conflict localEntity="${item.content}" storedEntity="${storedItem.content}" />
		</td>
	</tr>
