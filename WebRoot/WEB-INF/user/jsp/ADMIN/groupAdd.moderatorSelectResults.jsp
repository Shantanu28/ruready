...
<html:xhtml />
<c:set var="pagePrefix" value="${flowKeyPrefix}.moderatorSelectForm.results"/>
...
<div id="tableContent" class="wide">
	<c:choose>
		...
		<c:otherwise>
			<div class="data">
				<c:set var="pageBean" value="${currentFormObject.userSearch.pagingBean}"/>
				<table cellspacing="0">
					<caption>
						<span>
					...
					<tbody>
						<c:forEach var="user" items="${results}">
							<tr>								
								<td class="link">
									<html:link href="?_flowExecutionKey=${flowExecutionKey}&_eventId_assign&userId=${user.id}">
