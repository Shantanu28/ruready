...
<div id="tableContent" class="wide">
	<c:choose>
		...
		<c:otherwise>
			<div class="data">
				<c:set var="pageBean" value="${currentFormObject.search.pagingBean}"/>
				<table cellspacing="0">
					<caption>
						<span>
					...
					<thead>
						<tr>
							...
							<th><bean:message key="${fieldPrefix}.PRIMARY_MODERATOR.label"/></th>
						</tr>
					</thead>
					<tbody>
						<c:forEach var="group" items="${results}">
							<tr>								
								<td class="link">
									<html:link href="?_flowExecutionKey=${flowExecutionKey}&_eventId_subscribe&groupId=${group.id}">
										<bean:message key="${pagePrefix}.subscribe.altText"/>
									</html:link>
								</td>
								<td>${group.name}</td>
								<td>${group.course.description}</td>
								<td>${group.school.description}</td>
								<td>${group.primaryModerator.moderator.user.name.formattedName}</td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
