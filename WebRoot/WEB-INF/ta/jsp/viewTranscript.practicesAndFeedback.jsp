		...
		<table cellspacing="0">
			...
			<tbody>
				<c:forEach var="level" begin="1" end="4">
					<tr>
						<td class="link">
							<%-- if the recommended level matches this level, show the try me link here --%>
							<c:if test="${viewBean.transcript.recommendedLevel eq level}">
								<div id="tryMeNext">
						...
						<td>
							<c:if test="${viewBean.transcript.recommendedLevel ge level}">
								<c:choose>
									<c:when test="${viewBean.hasOpenPractice[level-1]}">
										<html:link href="?_flowExecutionKey=${flowExecutionKey}&_eventId_continuePracticeAssessment&level=${level}">
											<bean:message key="${fieldPrefix}.CONTINUE_ACTION.linkText"/>
										</html:link>
										<html:link href="?_flowExecutionKey=${flowExecutionKey}&_eventId_closePracticeAssessment&level=${level}">
											<bean:message key="${fieldPrefix}.CLOSE_ACTION.linkText"/>
										</html:link>
									</c:when>
									<c:otherwise>
										<html:link href="?_flowExecutionKey=${flowExecutionKey}&_eventId_startPracticeAssessment&level=${level}">
